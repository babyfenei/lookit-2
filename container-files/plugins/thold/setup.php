...
function thold_api_device_save ($save) {
	...
	if (isset($_POST['thold_send_wechat'])) {
		$save['thold_send_wechat'] = form_input_validate($_POST['thold_send_wechat'], 'thold_send_wechat', '', true, 3);
	} else {
		$save['thold_send_wechat'] = form_input_validate('', 'thold_send_wechat', '', true, 3);
	}

	if (isset($_POST['thold_host_wxuser'])) {
		$save['thold_host_wxuser'] = form_input_validate($_POST['thold_host_wxuser'], 'thold_host_wxuser', '', true, 3);
	} else {
		$save['thold_host_wxuser'] = form_input_validate('', 'thold_host_wxuser', '', true, 3);
	}
		...
		for ($i=0;($i<count($selected_items));$i++) {
			...
			if (count($existing) == 0 && count($template)) {
				...
				if ($graph) {
					...
					$insert['alert_wxusers_extra'] = $template['alert_wxusers_extra'];
...
function thold_host_edit_bottom() {
	...
	function changeNotify() {
		...
		if (document.getElementById('thold_send_wechat').value < 2) {
			document.getElementById('row_thold_host_wxuser').style.display = "none";
		}else{
			document.getElementById('row_thold_host_wxuser').style.display = "";
		}
	}
	-->
	</script>
