...
function form_save() {
	if (isset($_POST["save_component"])) {
		...
		$save["wxusers"] = form_input_validate($_POST["wxusers"], "wxusers", "", false, 3);
	...
	if (isset($_POST["selected_items"])) {
		if (isset($_POST["save_list"])) {
			...
			if ($_POST["drp_action"] == "1") { /* delete */
				...
				db_execute("UPDATE host SET thold_send_wechat=0 WHERE thold_send_wechat=2 AND " . array_to_sql_or($selected_items, "thold_host_wxuser"));
				db_execute("UPDATE host SET thold_send_wechat=1 WHERE thold_send_wechat=3 AND " . array_to_sql_or($selected_items, "thold_host_wxuser"));
				db_execute("UPDATE host SET thold_host_wxuser=0 WHERE " . array_to_sql_or($selected_items, "thold_host_wxuser"));
			...
			if ($_POST["drp_action"] == "1") { /* associate */
				for ($i=0;($i<count($selected_items));$i++) {
					...
					db_execute("UPDATE host SET thold_host_wxuser=" . get_request_var_request("id") . " WHERE id=" . $selected_items[$i]);
					/* set the global/list election */
					db_execute("UPDATE host SET thold_send_wechat=" . get_request_var_request("notification_action") . " WHERE id=" . $selected_items[$i]);
				}
				...
				for ($i=0;($i<count($selected_items));$i++) {
					...
					db_execute("UPDATE host SET thold_send_email=" . get_request_var_request("notification_action") . " WHERE id=" . $selected_items[$i]);
					/* set the notification list */
					db_execute("UPDATE host SET thold_host_wxuser=0 WHERE id=" . $selected_items[$i]);
					/* set the global/list election */
					db_execute("UPDATE host SET thold_send_wechat=" . get_request_var_request("notification_action") . " WHERE id=" . $selected_items[$i]);
				}
			}
				...
				for ($i=0;($i<count($selected_items));$i++) {
					...
					if ($_POST["notification_warning_action"] > 0) {
						...
						if ($_POST["notification_warning_action"] == 1) {
							...
							db_execute("UPDATE thold_template SET warning_wxusers_extra='' WHERE id=" . $selected_items[$i]);
					...
					if ($_POST["notification_alert_action"] > 0) {
						...
						if ($_POST["notification_alert_action"] == 1) {
							...
							db_execute("UPDATE thold_template SET alert_wxusers_extra='' WHERE id=" . $selected_items[$i]);
				...
				for ($i=0;($i<count($selected_items));$i++) {
					...
					if ($_POST["notification_warning_action"] > 0) {
						...
						if ($_POST["notification_warning_action"] == 1) {
							...
							db_execute("UPDATE thold_data SET warning_wxusers_extra='' WHERE id=" . $selected_items[$i]);
						}else{
							/* set the notification list */
							db_execute("UPDATE thold_data SET notify_warning=" . get_request_var_request("id") . " WHERE id=" . $selected_items[$i]);
						}
					...
					if ($_POST["notification_alert_action"] > 0) {
						...
						if ($_POST["notification_alert_action"] == 1) {
							...
							db_execute("UPDATE thold_data SET alert_wxusers_extra='' WHERE id=" . $selected_items[$i]);
...
function edit() {
	...
	if ($current_tab == "general") {
		...
		$fields_notification = array(
			...
			"wxusers" => array(
				"method" => "textarea",
				"friendly_name" => "Wechat Users",
				"description" => "Enter a comma separated list of Wechat Users for this notification list.",
				"value" => "|arg1:wxusers|",
				"class" => "textAreaNotes",
				"textarea_rows" => "4",
				"textarea_cols" => "80"
...
function hosts($header_label) {
	...
	if (sizeof($hosts)) {
		foreach ($hosts as $host) {
			...
			if ($host['thold_send_wechat'] == 0) {
				form_selectable_cell('<span style="color:blue;font-weight:bold;">Disabled</span>', $host['id']);
			}elseif ($host['thold_send_wechat'] == 1) {
				form_selectable_cell('<span style="color:purple;font-weight:bold;">Global List</span>', $host['id']);
			}elseif ($host['thold_host_wxuser'] == get_request_var_request('id')) {
				if ($host['thold_send_wechat'] == 2) {
					form_selectable_cell('<span style="color:green;font-weight:bold;">Current List Only</span>', $host['id']);
				}else{
					form_selectable_cell('<span style="color:green;font-weight:bold;">Current and Global List(s)</span>', $host['id']);
				}
			}elseif ($host['thold_host_wxuser'] == '0') {
				form_selectable_cell('<span style="color:green;font-weight:bold;">None</span>', $host['id']);
			}else{
				form_selectable_cell('<span style="color:red;font-weight:bold;">' . db_fetch_cell("SELECT name FROM plugin_notification_lists WHERE id=" . get_request_var_request('id')) . '</span>', $host['id']);
			}
...
function lists() {
	...
	if (strlen($_REQUEST['filter'])) {
		...
		emails LIKE '%%" . get_request_var_request("filter") . "%%' OR
		wxusers LIKE '%%" . get_request_var_request("filter") . "%%' )";
	} else {
		$sql_where = '';
	}
	...
	$lists = db_fetch_assoc("SELECT id, name, description, emails, wxusers
		FROM plugin_notification_lists
		$sql_where
	...
	$display_text = array(
        ...
        "wxusers" => array("Wechat Users", "ASC"));
	...
	if (sizeof($lists)) {
		foreach ($lists as $item) {
			...
			form_selectable_cell((strlen(get_request_var_request("filter")) ? preg_replace("/(" . preg_quote(get_request_var_request("filter")) . ")/i", "<span style='background-color: #F8D93D;'>\\1</span>", $item["wxusers"]) : $item["wxusers"]) . "</a>", $item["id"]);
