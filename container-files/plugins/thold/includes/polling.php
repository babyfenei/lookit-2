...
function thold_update_host_status () {
	...
	$alert_wxuser = read_config_option('alert_wxuser');
	$thold_enable_wechat = read_config_option('thold_enable_wechat');
				...
				if ($host['status'] == HOST_UP) {
					...
					switch($host['thold_send_email']) {
						...
						case '2': // Host List Only
							...
							$alert_wxuser = get_thold_notification_wxusers($host['thold_host_wxuser']);
							break;
						case '3': // Global and Host List
							$alert_email = $alert_email . ',' . get_thold_notification_emails($host['thold_host_email']);
							$alert_wxuser = $alert_wxuser . ',' . get_thold_notification_wxusers($host['thold_host_wxuser']);
							break;
					}
                    ...
                    if ($alert_wxuser == '' && $host['thold_send_wechat'] > 0) {
						cacti_log('THOLD: Can not send a Host Recovering WECHAT since the \'Alert WECHAT\' setting is not set !', true, 'POLLER');
					} elseif ($host['thold_send_wechat'] == '0') {
						cacti_log('THOLD: Did not send a Host Recovering WECHAT, disabled per host setting : ' . $host['description'] . ' !', true, 'POLLER');
					} elseif ($alert_wxuser != '') {
						thold_wechat($alert_wxuser, $msg, $subject);
						cacti_log('THOLD: Sent WECHAT ' . $host['description'] . ' !', true, 'POLLER');
                    }
	...
	if (count($hosts)) {
		foreach($hosts as $host) {
			...
			if ($alert_wxuser == '' && $host['thold_send_wechat'] > 0) {
				cacti_log('THOLD: Can not send a Host Down WECHAT since the \'Alert WECHAT\' setting is not set !', true, 'POLLER');
			} elseif ($host['thold_send_wechat'] == '0') {
				cacti_log('THOLD: Did not send a Host Down WECHAT, disabled per host setting : ' . $host['description'] . ' !', true, 'POLLER');
			} elseif ($alert_wxuser != '') {
				thold_wechat($alert_wxuser, $msg, $subject);
			}
		}
