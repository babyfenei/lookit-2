...
function thold_log($save){
	...
	if (read_config_option('thold_log_cacti') == 'on') {
		...
		$desc .= '  WECHAT_Sent_To: ' . $save['wxusers'];
...
function plugin_thold_log_changes($id, $changed, $message = array()) {
	...
	case 'modified':
		...
		//	if (count($rows)) {
		//		foreach ($rows as $row) {
		//		$alert_wxusers[] = $row['data'];
		//		}
		//	}
		//	$alert_wxusers = implode(',', $alert_wxusers);
			if ($alert_wxusers != '') {
				$alert_wxusers .= ',' . $warning_wxusers_extra['alert_wxusers_extra'];
			} else {
				$alert_wxusers = $thold['alert_wxusers_extra'];
			}
		}
		$alert_wxusers .= (strlen($alert_wxusers) ? ",":"") . get_thold_notification_wxusers($thold['alert_wxusers']);
		...
		$warning_wxusers = '';
		if (read_config_option('thold_disable_legacy') != 'on') {
			$warning_wxusers = $thold['warning_wxusers_extra'];
		}
		...
		if ($message['template_enabled'] == 'on') {
			...
			$desc .= '	Alert Wxusers: ' . $alert_wxusers;
			$desc .= '  Warning Emails: ' . $warning_emails;
			$desc .= '	Warning Wxusers: ' . $warning_wxusers;
...
function thold_check_threshold ($rra_id, $data_id, $name, $currentval, $cdef) {
	...
	//	$alert_wxusers = implode(',', $alert_wxusers);
		if ($alert_wxusers != '') {
			...
			$alert_wxusers = $item['alert_wxusers_extra'];
		}
	}
	$alert_wxusers .= (strlen($alert_wxusers) ? ",":"") . get_thold_notification_wxusers($item['alert_wxusers']);
	...
	$warning_wxusers = '';
	if (read_config_option('thold_disable_legacy') != 'on') {
		$warning_wxusers = $item['warning_wxusers_extra'];
	}

	$warning_wxusers .= (strlen($warning_wxusers) ? ",":"") . get_thold_notification_wxusers($item['warning_wxusers']);
	...
	case 0:	/* hi/low */
		...
		if ($breach_up || $breach_down) {
			...
			if ($notify) {
				...
				if (trim($alert_wxusers) != '') {
					thold_wechat($alert_wxusers, $msg, $subject);
				}
			...
			if ($notify) {
				...
				if (trim($warning_wxusers) != '') {
					thold_wechat($warning_wxusers, $warn_msg, $subject);
				}
				...
				if (trim($alert_wxusers) != '') {
					thold_wechat($alert_wxusers, $warn_msg, $subject);
				}
			...
			if ($alertstat != 0) {
				...
				if ($item['thold_warning_fail_count'] >= $warning_trigger && $item['restored_alert'] != 'on') {
					...
					if (trim($warning_wxusers) != '' && $item['restored_alert'] != 'on') {
						thold_wechat($warning_wxusers, $msg, $subject);
					}
					...
					if (trim($alert_wxusers) != '' && $item['restored_alert'] != 'on') {
						thold_wechat($alert_wxusers, $msg, $subject);
					}
...
function save_thold() {
	...
	input_validate_input_number(get_request_var_post('warning_wxusers'));
	input_validate_input_number(get_request_var_post('alert_wxusers'));
	...
	$save['alert_wxusers_extra'] = (trim($_POST['alert_wxusers_extra'])) == '' ? '' : $_POST['alert_wxusers_extra'];
	$save['warning_wxusers_extra'] = (trim($_POST['warning_wxusers_extra'])) == '' ? '' : $_POST['warning_wxusers_extra'];
	...
	$save['alert_wxusers'] = $_POST['alert_wxusers'];
	$save['warning_wxusers']   = $_POST['warning_wxusers'];
		...
		if (count($existing) == 0 && count($template)) {
			...
			if ($graph) {
				for ($y = 0; $y < count($template); $y++) {
					...
					$insert['warning_wxusers_extra'] = $template[$y]['warning_wxusers_extra'];
          $insert['alert_wxusers_extra'] = $template[$y]['alert_wxusers_extra'];
          ...
          $insert['warning_command'] = $template[$y]['warning_command'];
					...
					$insert['warning_wxusers']     = $template[$y]['warning_wxusers'];
					$insert['alert_wxusers']       = $template[$y]['alert_wxusers'];
...
/*
		...
		$message = str_replace('<BR>',  "\n", $message);
		$message = str_replace('</BR>', "\n", $message);
		$message = strip_tags($wx_msg);      //删除message中的html标签,并将<br>替换为换行符\n
	    $message = trim($wx_msg);                                        //整理msg_wx字符串
	    $subject = $subject;
		 /*$wx_msg = iconv( "GB2312//IGNORE", "UTF-8", $message);           //转换编码为utf-8，防止乱码
	     $subject = iconv( "GB2312//IGNORE", "UTF-8", $subject);          //转换编码为utf-8，防止乱码 
	     ...
	     if($f  = file_put_contents($file_message,  $message))            //将msg_wx参数的值保存到file_message中
	*/	
...
function thold_template_update_threshold ($id, $template) {
	db_execute("UPDATE thold_data, thold_template
		...
		thold_data.alert_wxusers_extra = thold_template.alert_wxusers_extra,
		thold_data.warning_wxusers_extra = thold_template.warning_wxusers_extra,
		thold_data.alert_command = thold_template.alert_command,
		thold_data.warning_command = thold_template.warning_command,
		thold_data.notify_warning = thold_template.notify_warning,
		thold_data.notify_alert = thold_template.notify_alert,
		
		thold_data.warning_wxusers = thold_template.warning_wxusers,
		thold_data.alert_wxusers = thold_template.alert_wxusers,
...
function get_thold_notification_wxusers($id) {
    if (!empty($id)) {
        return trim(db_fetch_cell('SELECT wxusers FROM plugin_notification_lists WHERE id=' . $id));
    } else {
        return '';
    }
}

function thold_wechat($users, $message, $subject) {
    global $debug;
	thold_debug('test wechat!');
	...
	*/
    ...
    $command_return = 999;
    $thold_sendwechat_path = trim(read_config_option('thold_sendwechat_path'));

    $wechat_users = explode(',', $users);
    ...
    if (count($wechat_users) == 0) {
        thold_debug('DEBUG: thold_wechat: no users defined, do nothing');
        return;
    }
    thold_debug('Sending WECHAT to wxusers: ' . $warning_wxusers);
    foreach($wechat_users as $key => $value) {
        if (strlen($thold_sendwechat_path)>2) {
            $command = 'bash ' . $thold_sendwechat_path . ' ' . trim($value) . ' "' . $message . '"' .' "' . $subject . '"' ;
   		 } else {
    	}    	exec($command, $command_output, $command_return);

        thold_debug('DEBUG: thold_wechat: command ==>' . $command, true, 'POLLER');
        foreach ($command_output as $key => $value) {
            thold_debug('DEBUG: thold_wechat: command output ==> ' . $key . ' = ' . $value, true, 'POLLER');
        }
        thold_debug('DEBUG: thold_wechat: command return value ==> ' . $command_return, true, 'POLLER');
    }
}
